...
class AbstractApplicationController extends AbstractActionController
{
    ...
    public function onDispatch(MvcEvent $e)
    {
        ...
        if (! $this->identity() || ! $this->identity() instanceof Entity) {
            if (! $this->acl()->checkAcl('guest', $router->getMatchedRouteName())) {
                return $this->redirect()->toRoute('auth-sign-in');
            }
        } else {
            if (! $this->acl()->checkAcl($this->identity()->getAclRoleEntity()->getAclRoleName(), $router->getMatchedRouteName())) {
                return $this->redirect()->toRoute()->setStatusCode(403);
            }
        }
